...
# Request locking to prevent duplicate outgoing API calls
_API_LOCKS = {}
...
def fetch_data(url: str, headers: dict) -> dict | None:
    ...
    try:
        ...
        _LOGGER.error(f"Exception occurred while fetching data from {url}: {str(e)}")
        return None
...
def get_api_lock(key: str) -> threading.Lock:
    """Get a lock object for the specified API request key.
    
    Args:
        key (str): A unique key identifying the API request
        
    Returns:
        threading.Lock: A lock object for synchronizing API calls
    """
    if key not in _API_LOCKS:
        _API_LOCKS[key] = threading.Lock()
    return _API_LOCKS[key]
...
def handle_add_media(hass: HomeAssistant, call: ServiceCall, media_type: str, service_name: str) -> None:
    ...
    # Get a lock for this specific media addition
    lock_key = f"{service_name}_{media_type}_{title}"
    api_lock = get_api_lock(lock_key)
    
    # Try to acquire the lock with timeout
    if not api_lock.acquire(timeout=0.1):
        _LOGGER.warning(f"Another request for '{title}' in {service_name} is already in progress. Skipping duplicate request.")
        return
    ...
    try:
        ...
        if media_list:
            ...
            _LOGGER.info(f"Adding media to URL: {add_url}")
...
def handle_add_overseerr_media(hass: HomeAssistant, call: ServiceCall, media_type: str) -> None:
    ...
    # Get a lock for this specific overseerr request
    lock_key = f"overseerr_{media_type}_{title}"
    api_lock = get_api_lock(lock_key)
    
    # Try to acquire the lock with timeout
    if not api_lock.acquire(timeout=0.1):
        _LOGGER.warning(f"Another request for '{title}' in Overseerr is already in progress. Skipping duplicate request.")
        return
    ...
    try:
        ...
        if search_results and search_results.get("results"):
            ...
            _LOGGER.info(f"Creating request with URL: {request_url}")
